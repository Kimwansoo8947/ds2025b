...
# 상수시간  -> 로그시간 -> 선형시간 -> 선형 로그 시간 -> 2차 시간 -> 3차 시간 -> 지수 시간 -> 팩토리엏
...
# 배열에 저장된 각 요소는 인덱스를 통해 접그 할 수 있다.
...
# 세트의 중복 요소 찾기
# 세트는 중복을 허용하지 않기 때문에 이터러블 (순환하는) 데이터에서 요소를 하나씩 추가할때, 세트의 길이가 그래로라면 추가할때, 세트의 길이가 그대로라면 요소가 중복이라는 것을 알수 있다.
